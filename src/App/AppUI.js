...
function AppUI() {
  ...
  return (
    // Esto es lo que retorna nuestro Componente, son sus elementos internos, NO ES UN COMPONENTE, lo de abajo NO ES HTML, es JSX una sintaxis que facilita la lectura de código y luego se reenderiza a HTML clásico
    <>
      {' '}
      ...
      <ToDoList>
        ...
        {/* Mostramos un mensaje donde decimos que no hay ningun ToDo creado, se muestra si no estamos en estado de 'loading' y 'searchedToDos es igual a 0' */}
        {!loading &&
          searchedToDos.map(
            (
              toDo // Retorna y muestra los ToDos que fueron buscados en el buscador, si el buscador esta vacío muestra todo los ToDos guardados
            ) => (
              <ToDoItem
                key={toDo.text}
