...
function useToDos() {
  // Convertiremos este Provider en un Custom Hook, ya que no envolvera al Componente App ya no es necesario aplicar la lóciga de desctructuración de 'children'. Este Custom Hook puede consumir otros Custom Hooks como 'useLocalStorage'
  const {
  ...
  const searchedToDos = toDos.filter((toDo) =>
    toDo.text.toLowerCase().includes(
      // Guardamos en un Array los ToDos que coinciden con un 'searchValue'
      searchValue.toLowerCase()
    )
  )

  const addToDo = (text) => {
    // Funcion que agrega un nuevo ToDo al Array de Objecto con los 'newToDos', recibe el valor del 'text' del nuevo ToDo
    const newToDos = [...toDos] // Hacemos una copia que no afecta el valor por referencia del Array 'toDos' original
    newToDos.push({
      // Agregamos un nuevo Object al Array con el value del 'text' y la propiedad de 'completed' a false por defecto ya que es un ToDo recien agregado
      text,
      completed: false,
    })
  ...
  const validatingToDo = (text) => {
    ...
    if (toDoIndex !== -1) {
      // Si ya existe el texto de ese ToDo guardado le decimos al usuario que no puede ingresar ToDos repetidos
  ...
  const completeToDo = (text) => {
    // Función que cambia el State de 'toDos' en el apartado de 'completed' de false a true
  ...
  const deleteToDo = (text) => {
    // Función que crea un nuevo Array de los toDos sin el ToDo que le dimos X para eliminar
  ...
  return {
    // En vez de retornar los valores, States, y funciones como un 'value' del Provider de un Context, ahora simplemente los devolvemos como un Objeto normal para que el Componente App lo consuma haciendo Composición de Componentes
    loading,
    error,
    completedToDos,
    totalToDos,
