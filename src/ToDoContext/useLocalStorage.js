...
function useLocalStorage(itemName, initialValue) {
  // Definimos un 'Custom Hook' para acceder a los datos guardados en LocalStorage
  ...
  React.useEffect(() => {
    // El 'useEffect()' hook recibe una funcion y un Array, si es un Array vacio solo se ejecuta una vez
    setTimeout(() => {
      // Simulamos una solicitud a una API de 500 milisegundos de tiempo de espera
      try {
        ...
        let parsedItem
        if (!localStorageItem) {
          // Si no devolvio ningun valor entonces le ponemos como 'initialValue' un Array vacío
  ...
  const saveItem = (newItem) => {
    // Función para guardar un nuevo ToDo en el LocalStorage y actualizar el State
